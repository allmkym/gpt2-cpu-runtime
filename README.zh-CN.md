...
一个面向单机 CPU、基于 C++20 和 FP32 的 GPT-2 inference runtime。项目包含 full-prefix correctness baseline、incremental KV-cache path，以及针对 single-row Linear 的可选 persistent worker executor。输入和输出均为 token ID；仓库不包含 model weights 或 tokenizer。

## 项目来源

本仓库是基于 [karpathy/llm.c](https://github.com/karpathy/llm.c/tree/f1e2ace651495b74ae22d45d1723443fd00ecd3a) 中 GPT-2 CPU inference implementation 发展而来的 C++20 follow-on runtime，并沿用了 JYY OS 2026 M6 所采用的 inference-focused subset；我此前将 M6 作为 C systems/concurrency exercise 完成。本仓库不包含我原始的 M6 submission 或 course framework。在此基础上，项目进一步加入了明确的 C++ ownership / lifetime boundaries、incremental KV-cache inference、persistent linear executor，以及额外的 correctness / performance validation。
...
- **Numerical validation：** 使用 synthetic tests、real-checkpoint comparisons，以及固定版本的独立 `llm.c` oracle；比较的是完整 logits vector，而不仅是最终生成的 token ID。
...
这两组数据来自两个不同 experiment，不能相乘，也不能视为同一个 factorial comparison。M2 的提升来自整条 cached path 的综合变化，包括 KV reuse、one-token execution，以及 last-position real-vocabulary projection；M3 则是在同一最终 build 中，对 optional parallel linear executor 进行 1-thread 与 8-total-thread 的对比。每组 median 均采用 1 次 warmup 和 5 次 timed repetitions。Model loading、session creation、executor construction、logging 和 logits dump 均不计入 generation timing。

M3 的 thread-count sweep 测试了 1、2、4、8、16 total threads；在这组 workload 上 8 threads 最好，而 16 threads 出现 regression。因此这些结果只代表该机器与该 workload 下的 bounded local measurements，不应视为 portable thread-count recommendation。Raw samples 和额外 workload 见 [实现与验证说明](IMPLEMENTATION_NOTES.md) 与 [M3 结果记录](docs/MILESTONE3_RESULTS.md)。

## Build / Run

要求：CMake 3.20+、支持 threads 的 C++20 compiler；启用 tests 时需要 Python 3.10+。记录中的 build environment 为 WSL2 下的 Ubuntu + GCC。
...
默认 configuration 会运行不依赖 checkpoint 的 synthetic tests 与 CLI tests。如需运行 real-model integration test，在 configure 阶段提供兼容的 GPT-2 checkpoint：
...
当 checkpoint 文件存在时，test list 中应包含 `gpt2_real_checkpoint`。使用该 checkpoint 时，记录中的 final-source Release、ASan/LSan/UBSan，以及单独的 TSan suites 均通过 6/6。Sanitizer runs 见 [M3 结果记录](docs/MILESTONE3_RESULTS.md)；这些 runs 与 Release timing build 相互独立。
...
使用 KV cache 和 optional parallel linear executor：
...
> 更深入的 design、validation 和 raw benchmark records 继续维护单一英文版本，以避免中英文 evidence documents 发生漂移。
